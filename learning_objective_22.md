...
Let’s start with enumerating SQL servers in the domain and if `student422` has privileges to connect to any of them. We can use PowerUpSQL module for that. Run the below command from a PowerShell session started using Invisi-Shell.
...
ComputerName           : dcorp-mssql.dollarcorp.moneycorp.local🗄️
...
So, we can connect to `dcorp-mssql`. Using HeidiSQL client, let’s login to `dcorp-mssql` using windows authentication of `student422`.
...
If `xp_cmdshell` is enabled (or RPC out is true, which is set to false in this case), it is possible to execute commands on `eu-sql26` using linked databases. To avoid dealing with a large number of quotes and escapes, we can use the following command.
...
Let's try to execute a PowerShell download execute cradle to execute a PowerShell reverse shell on the `eu-sql26` instance. Remember to start a listener.
...
![Victim: eu-sql26 | system](https://custom-icon-badges.demolab.com/badge/eu--sql26-system-64b5f6?logo=windows11&logoColor=white)
